...
For a full-history build, `financial_statement_pit_selected` and `self_reported_numbers_pit_selected` are **as-of state-update** artifacts, not repeated daily snapshot grids. A row is emitted only when a selected logical key changes, at the first requested trading decision date on or after the source availability date. To reconstruct a decision date, consumers must take the latest emitted row for each logical key with `decision_date <= requested_decision_date`; they must never select a later event. This is PIT-equivalent to a daily snapshot while avoiding repeated copies of unchanged statements.
