...
Before fitting a declared model in every outer fold, verify every declared numeric feature has at least one non-missing training observation in that fold. Do not allow an imputer or estimator to silently drop an all-missing declared feature. If this fails, preserve any already-created immutable output only as `INVALID_INPUT_COVERAGE`, record no performance conclusion, and create no Tier 1 gate admission. A replacement feature set is a new trial and must increase the effective DSR trial count conservatively.
...
Test hand-calculated `0.001425/0.003` costs/barriers, confirm that Tier 1 never applies a one-NTD ticket minimum without a concrete order notional, raw-OPEN timing, daily-close trigger timing, prohibited close-path-OHLC feature exclusion, unresolved tails, future append invariance, source delay, purging/embargo, and absence of future columns. For `08`, test per-ticket `max(notional * 0.001425, 1 NTD)`, integer-share rounding, no-order cases, and one-and-only-one interpretation of the `0.003` sell-side rate. The hand-off to `07` contains only PIT features, OOF/walk-forward `p1`, candidate indicator, decision time, event identifiers, and immutable lineage. In-sample Tier 1 predictions are forbidden in this hand-off.
